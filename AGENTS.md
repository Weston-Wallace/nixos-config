...
declaratively defines the entire system and user environment for two
machines owned by user **westonw**:
...
hosts/
  ...
  scar/
    default.nix                  # ASUS server host-specific settings
    hardware-configuration.nix   # Placeholder in repo; replace with generated file on target
modules/
  common/
    base.nix                     # Shared system defaults (boot, locale, nix settings, core packages)
    user-westonw.nix             # Shared westonw user account baseline
  roles/
    desktop.nix                  # Desktop-only stack (Hyprland, Stylix, audio, bluetooth, fonts)
    server.nix                   # Headless server defaults (SSH, trim, smartd, no GUI stack)
  services/
...
# Rebuild and switch (scar)
sudo nixos-rebuild switch --flake ~/nixos-config#scar

# Dry-run build (user runs manually)
sudo nixos-rebuild dry-build --flake ~/nixos-config#nullrunner

# Dry-run build (scar)
sudo nixos-rebuild dry-build --flake ~/nixos-config#scar
```
